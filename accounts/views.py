...
def auth0_login_view(request):
    ...
    WhatsApp+OTP signup/login flow. No `connection` param is forced, so
    Auth0's own page shows both the Google button and its Database
    (email/password, with built-in forgot-password) option."""
    if not settings.AUTH0_DOMAIN:
        messages.error(request, "تسجيل الدخول عبر جوجل أو البريد الإلكتروني غير مفعّل حالياً.")
