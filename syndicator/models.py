...
class AISettings(models.Model):
    ...
    support_whatsapp_number = models.CharField(max_length=50, blank=True, null=True, verbose_name="رقم واتساب الدعم الفني", help_text="الرقم الذي يظهر للعملاء للتواصل المباشر (زر واتساب عائم في الموقع وصفحة تواصل معنا). اكتبه بصيغة دولية بدون + أو مسافات (مثال: 201099437596). اتركه فارغاً لإخفاء زر التواصل.")
    enable_paypal_gateway = models.BooleanField(default=True, verbose_name="إظهار PayPal كطريقة دفع")
    enable_paymob_gateway = models.BooleanField(default=True, verbose_name="إظهار Paymob (بطاقات مصرية) كطريقة دفع")
    enable_local_wallet_gateway = models.BooleanField(default=True, verbose_name="إظهار المحافظ المحلية (فودافون كاش) كطريقة دفع")
...
class WordPressSite(models.Model):
    ...
    social_logo_position = models.CharField(max_length=20, choices=SOCIAL_LOGO_POSITION_CHOICES, default='top_left', verbose_name="مكان اللوجو في التصميم", help_text="يتحكم في مكان لوجو الموقع في قالبي \"إخباري احترافي\" و\"عاجل\" فقط.")
...
class WPConnectionToken(models.Model):
    ...
    customer = models.ForeignKey(
        'accounts.CustomerProfile', on_delete=models.CASCADE, null=True, blank=True,
        related_name='connection_tokens', verbose_name="العميل المالك",
        help_text="مصدر الملكية الفعلي للكود — لا تُستخدم client_name للتحقق من الملكية",
    )
